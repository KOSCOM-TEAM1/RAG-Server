...
- 베이스 모델로 사용되는 `gpt-4o-mini`와 같은 대규모 언어 모델(LLM)은 `2023-10`까지의 데이터로 사전 학습됩니다. 이로 인해 모델이 학습한 시점 이후의 최신 정보나 특정 도메인에 대한 깊이 있는 정보는 알지 못하는 한계가 있습니다.
...
- **Response Body**:
    ```json
    {
      ...
      "referenced_cases": [
        {
          "content": "과거 유사 사례 1의 뉴스 내용",
          "score": 0.9587
        },
        {
          "content": "과거 유사 사례 2의 뉴스 내용",
          "score": 0.8912
        }
      ]
    }
...
- **Response Body**:
    ```json
    {
      "stock": "삼성전자",
      "decision_report": "결정: 매수\\n이유: 1. ... 2. ...",
      "referenced_cases": ["순수 LLM의 일반 지식으로만 분석"]
    }
    ```